...
int main ( int argc, char * argv[] ) {
  ...
  // TEST 1: Check mag(3+4j) ~= 5 and angle(3+4j) ~= 0.927
  complex x(3, 4);
  ASSERT(TOLERANCE > abs(x.mag() - 5.0));
  ASSERT(TOLERANCE > abs(x.angle() - 0.927))

  // TEST 2: Check mag(-2.34-6.98j) ~= 7.36179 and angle(-2.34-6.98j) ~= -1.89427
  complex y(-2.34, -6.98);
  ASSERT(TOLERANCE > abs(y.mag() - 7.36179));
  ASSERT(TOLERANCE > abs(y.angle() + 1.89427));
  ...
  complex z(-0.0, -0.0);
  ASSERT(TOLERANCE > abs(z.mag() - 0.0));
  if (!(TOLERANCE > abs(z.angle() + PI))) {
